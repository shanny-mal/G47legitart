...
/* Removed the explicit /login item here — header handles Sign in/out */
...
const NavLinks: React.FC<{ className?: string }> = React.memo(({ className = "" }) => {
  ...
  return (
    <nav aria-label="Primary navigation" className={`hidden md:flex items-center gap-4 ${className}`}>
      {links.map((l) => {
        ...
        return (
          <Link
            ...
            className={`relative px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 ${
              active ? "text-karibaNavy dark:text-karibaSand" : "text-gray-700 dark:text-gray-200 hover:text-karibaTeal"
